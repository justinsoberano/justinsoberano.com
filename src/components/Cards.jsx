...
export function Experience(toggle) {

    const [show, setShow] = useState(false);
    useEffect(() => { setShow(!toggle) }, !toggle)

    function CardSpring() {
        return useSpring({
            from: show ? { opacity: 0, transform: "translateY(400px)" } : { opacity: 1, transform: "translateY(0px)" },
            to: show ? { opacity: 1, transform: "translateY(0px)" } : { opacity: 0, transform: "translateY(400px)"},
            delay: 0,
